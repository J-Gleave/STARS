      ...
      REAL*8 SDM, QPC, RCO, ETH, F8, QPP, TRB, DABS
      REAL*8 SF, R33, AVM, VME, MWT, DH2, DELTA_H, HNUC
      REAL*8 ACCR, PERIOD, AAWT, STAT2, Q, MONTAG, YUK1, Q33
      ...
      REAL*8 EC, MSUN, VMH, SODDS, QAO, WA, STC, RANE
      REAL*8 TMAX, SEP, STM, TINTBASE, XO, VLHP, AGE
      REAL*8 RBE, XN, PF, VLE, RMG, QGMG, RCB, XINIT
      REAL*8 DLOG, MAXB, E20, CH, MHC, QCO, DT, XMG
      REAL*8 PMESH, PT, VLCP, CL, CT, AK2, RLFP, FK
      REAL*8 RHO, YA, ROO, TM, VM, EPS, DINF
      REAL*8 VI, LE, S, ALPHA, QBE, CM, RGMG, R
      REAL*8 RAN, DTY, MOD, MIX, VLEP, CMEVMU, QANE, TADJ
      REAL*8 DSEP, VLT, CA, SWIND, EN, RPC, CPL
      REAL*8 QAC, CPI, RAC, LT, RMT, TRANS, QQ, AK1
      REAL*8 VMC, HT, E12, BM, OCRIT, RSUN, CNSTS, MZAMS
      REAL*8 TOTMP, RW, VX, ZAMS, WF, ENX, PX, FR
      REAL*8 HSPINTOT, CEN, SG, TCB, TRC1, RLFCHECK1, TC
      ...
      REAL*8 PS, DEXP, AC, CG, XH, OS, Q3A, VALUE
      REAL*8 EXLIM, ABUND, SQRT, ALPHACE, U, AM, RLFC, XNE
      REAL*8 P, A1, XSI, TRC2, CP, XASH, PER0, CEVB
      REAL*8 LHEF, E14, TIDES, WIND, MTA, RPNG, OMG
      REAL*8 WWW, CB, VLN, GRAD, LKP, VLEC, VLCC
      REAL*8 CR, A2, E10, DR, XHE, QGNE, EVMODE, A5
      REAL*8 CEE, BY, RPO, OSPIN, PWT, OORB, EQ
      REAL*8 QAN, QBP, QPO, WMHE, RLFCHECK2, MAXB2, A4
...
!     Decide whether to print the interior or not
            ...
            END IF
            IF (NPRINT.EQ.1.AND.IW(102).NE.0) THEN
                  WRITE(36+20*(ISTAR-1),*) NMOD
                  WRITE(37+20*(ISTAR-1),*) NMOD
                  WRITE(38+20*(ISTAR-1),*) NMOD
                  WRITE(36+20*(ISTAR-1), 99001) (BX(J), J=1,16)
                  WRITE(37+20*(ISTAR-1), 99001) (CX(J), J=1,16)
                  WRITE(38+20*(ISTAR-1), 99001) (DX(J), J=1,17)
            END IF
            DO KK = 1, NMESH
                  K = NMESH + 1 - KK
                  DO J = 1, 20
                        IF (IEND.LT.0) THEN
                              QD(J) = 0.0D0
                              Q(J) = H(J+15*(ISTAR-1), K)
                        ELSE
                              QD(J) = DH(J+15*(ISTAR-1), K)
                              Q(J) = H(J+15*(ISTAR-1), K) + QD(J)
                        END IF
                  END DO
                  IF (ISTAR.EQ.2) THEN
                        Q(28) = H(13, K)
...
! locate boundaries of burning regions (EX > EXLIM)
                  ...
                  PX(22) = PX(9)-SX(9,KK)
                  WF = SX(17,KK)/PX(17)
                  PX(23) = (SX(9,KK)*SX(23,KK)*WF**2+PX(22)*0.4D0*(1.0D0-WF**5) / (1.0D0-WF**3))/PX(9)
...
! Print the interior details on first `page', if required
                  ...
                  IF (NPRINT.EQ.1.AND.MOD(K,10*NWRT2).EQ.0) THEN
                        WRITE(32+20*(ISTAR-1),99002)
                  END IF

                  IF (NPRINT.EQ.1.AND.MOD(K-1,NWRT2).EQ.0.AND.IW(102).NE.0) THEN
     ...
     :                  PX(9)
                  END IF
                  IF (NPRINT.EQ.1.AND.MOD(K,10*NWRT2).EQ.0.AND.IW(102).NE.0) THEN
...
! Write further `pages' for each detailed model, if required
                  DO I=2, NWRT3
                        IJ = 15*I-15
                        IF (NPRINT.EQ.1) THEN
                              WRITE(32+20*(ISTAR-1), 99001) (AX(ISX(J+IJ)), J=1,15)
                        END IF
                        DO KK = 1, NMESH
                              K = NMESH + 1 - KK
...
! If boundary moves in, reduce DD
                  IF (IDREDGE.EQ.2.AND.VLEC(ISTAR).LT.1d5) THEN
                        MAXB2 = 0d0
                        DO II = 1,12
                              IF (MCB(II).GT.MAXB2.AND.MCB(II).LT.1d0) THEN
                                    MAXB2 = MCB(II)
                              END IF
                        END DO
                        IF (MAXB2.LT.MAXB) THEN
                              IDREDGE = 3
                        END IF
                  END IF
...
!           Output accplot and overflow file - JLG 14/07/2025
...
! Find convective envelope base
            ...
            DO K=NMESH-1,1,-1 !surf to centre
                  IF (SX(8,K+1).GT.0d0.AND.SX(8,K).LT.0d0.AND.SX(9,K).GT.MENV(ISTAR)/MSUN.AND.K.LT.NMESH-20) THEN
