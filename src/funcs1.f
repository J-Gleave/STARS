...
! rest of this subroutine. RJS 5/7/06
     ...
     &                CEN, CPL, CMEVMU, CSECYR, LSUN, MSUN, RSUN, TSUNYR
      ...
      COMMON /ACCDAT/ ACCDAT1(6), ACCOMPOS(7,31, 2), ACCDAT2(7)
      COMMON /EVMODE/ IMODE
      COMMON /INERTI/ VI(2)
      ...
      COMMON /OVRFLW/ HP, SX(45,MAXMSH+1), OVRFLW1
...
!            CT(2) = 0.0
     ...
     &                    + CT2*LOG(P+CP1)
     &                    + CT2*LOG(P+CP2)
!     &                    + 0.1*LOG(P+CP4) ! Carbon term, currently not working (may need to create a smart prefactor like the new Helium prefactor (CT2)

            VPP = CT(4) + CT(5)*P/(P+CP3) 
     &                  + CT2*P/(P+CP2)
     &                  + CT2*P/(P+CP1)
!     &                  + 0.1*P/(P+CP4) ! Carbon term, currently not working (may need to create a smart prefactor like the new Helium prefactor (CT2)
      END IF
...
! Updated to vary with primary mass and for age condition to match massloss - JLG 17/09/2025
            IF (AGE.LT.1d3) THEN
                  RLF = -1d1/(M/MSUN)**2 ! -1d-1
            END IF
      END IF
